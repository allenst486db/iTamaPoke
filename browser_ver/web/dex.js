...
// renderGalleryDetail -- minus the sprite-atlas thumbnails (TPThumbs) and
// the dex-entry text page (TPDexEntryText, which reads a user-supplied
// mons/dex_entries_<lang>.txt -- not ported yet, see roadmap). Tiles show
// the dex number and a registered/caught/unknown colour instead of a
// silhouette, and detail shows name/number/types/obtained-via only.
...
function dexGridTap(x, y) {
  ...
  if (y >= GAL_Y && y < GAL_Y + 4 * GAL_CELL) {
    ...
    if (c >= 0 && c < 4 && r >= 0 && r < 4) {
      const dex = dexAt(dexPage * 16 + r * 4 + c);
      if (dex > 0) { dexDetailDex = dex; dexScreen = "detail"; return; }
    }
  }
  if (y >= 448 && y < 464) {
    const pages = dexPageCount();
