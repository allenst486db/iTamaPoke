...
- **아이폰 / 아이패드**: 링크를 **Safari**로 엽니다(Chrome이나 앱 내장
  ...
  **홈 화면에 추가** → **추가**.
- **안드로이드**: 링크를 **Chrome**으로 엽니다. **⋮** 메뉴 → **앱 설치**(또는
  **홈 화면에 추가**) → **설치**.

홈 화면에 **iTamaPoke** 아이콘이 생깁니다. 브라우저는 닫고, 이제부터는 그
아이콘으로 게임을 여세요.
...
플레이하는 데는 필요 없습니다. 위 링크의 사이트는
[`.github/workflows/build-browser.yml`](../.github/workflows/build-browser.yml)이
푸시마다 빌드·배포합니다(Emscripten이 `upstream-expanded/` +
`browser_ver/core/`를 WASM으로 컴파일하고, 폴더를 GitHub Pages에 올림).
본인 fork에서 배포하려면 **Settings → Pages → Source: GitHub Actions**로 두고
워크플로를 한 번 실행하세요. 로컬 빌드는 `browser_ver/README.md`의 emsdk
절차를 따르면 됩니다. LICENSE 3조 참고: 이 빌드 사본을 다른 곳에 다시
호스팅하는 건 허가가 필요합니다. 워크플로는 저작권자 본인의 주소 하나를
모두가 쓰게 하려고 있는 것입니다.
