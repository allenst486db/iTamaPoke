...
세 번째 버튼도 같은 방식입니다: `Scripts/fetch_cries.sh`로(실행하는
컴퓨터에 `ffmpeg`가 필요합니다 — README의 "울음소리" 참고)
`psnd<도감번호>.m4a`를 받은 뒤 **"Load cries…"**를 눌러 다중 선택합니다.
그러면 도감 상세화면 첫 페이지의 초상화 아래에 재생 캡슐이 생기고, 누르면
그 종의 울음소리가 재생되면서 캡슐이 차오릅니다(다시 누르면 정지). 파일을
넣지 않은 종은 버튼 자체가 안 보입니다. 울음소리는 사운드 설정을 따릅니다 —
소리+진동(ALL)일 때만 재생되고, 진동만/무음에서는 재생되지 않습니다(iOS 앱과
동일한 규칙).
