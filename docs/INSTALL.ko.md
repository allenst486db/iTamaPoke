...
만든 것(`browser_ver/`)입니다 — GitHub가 본인 계정 아래에 한 번 빌드해
주면, 그 뒤로는 본인 링크에서 설치해 폰 혼자서 오프라인으로 돌아갑니다.
Mac도, 개발자 계정도, 사이드로딩 도구도 필요 없어서 **처음이라면 경로 D를
권합니다.**
...
## 경로 D — 내 fork에서 만드는 웹앱 / 안드로이드 앱 (Mac 없이, 개발자 계정 없이)

같은 게임 — 같은 엔진, 같은 화면, 같은 스프라이트, 같은 소리 — 을 웹앱과
안드로이드 패키지로 만든 것입니다. GitHub가 본인 계정 아래에 한 번
빌드해 주면(D0), 폰에서 본인 링크를 열어 홈 화면에 추가(또는 APK 설치)하고,
그 뒤로는 다른 앱처럼 **폰 혼자서, 오프라인으로** 돌아갑니다. App Store도,
사이드로딩도, 7일 만료도, 어디선가 켜둬야 하는 것도 없습니다. 세이브는 따로
갖습니다(iOS 앱과 공유하지 않습니다).

그 링크에 있는 건 코드뿐입니다. 포켓몬 그림·울음소리·도감 설명문은 **들어
있지 않고**, D3에서 본인 폰에 직접 넣으면 앱이 폰 저장소에서 읽습니다.
그래서 게시가 가능한 겁니다 — Nintendo 것도, 스프라이트 작가들 것도
서버에서 나가는 게 없습니다.

**공용 링크는 없습니다.** 이 저장소는 게임을 모두에게 호스팅하지 않습니다.
대신 D0에서 GitHub가 *본인 계정 아래에* 사본을 한 번 빌드해 줍니다. 그러면
이 저장소에 무슨 일이 생겨도 계속 동작하는 본인만의 링크
(`https://<내아이디>.github.io/iTamaPoke/`)가 생기고, LICENSE가 허용하는
형태(본인용 개인 빌드, 남에게 돌리는 링크는 아님)이기도 합니다. 어느
쪽이든 게임이 남과 공유되는 일은 없습니다: 캐릭터·레벨·도감·포획
기록·스프라이트는 폰의 앱 저장소 안에만 있고 계정도 서버 저장도 없습니다.
...
2. 오른쪽 위 **Fork** → **Create fork**. 이제
   `github.com/<내아이디>/iTamaPoke`에 본인 사본이 생겼습니다. 공개
   상태로 두세요 — 무료 계정의 GitHub Pages는 공개 저장소에서만 됩니다.
...
4. **Actions** 탭 → **I understand my workflows, go ahead and enable them**
   클릭 → 왼쪽 **Build (browser)** → **Run workflow** → **Run workflow**.
5. 초록색 체크 ✅가 뜰 때까지(약 5분, 웹앱과 안드로이드 패키지를 같이
   빌드) 기다립니다. 링크는 **`https://<내아이디>.github.io/iTamaPoke/`**
   이고, 실행 페이지의 *publish* 아래에도 표시됩니다.

이후 이 저장소의 변경을 가져오려면 본인 사본에서 **Sync fork** → **Update
branch**. 사이트는 알아서 다시 빌드되고, 설치된 앱은 다음 실행 때
업데이트됩니다.

### D1. 설치 (1분)

- **아이폰 / 아이패드**: 본인 링크를 **Safari**로 엽니다(Chrome이나 앱 내장
...
- **안드로이드**: 진짜 앱으로 설치합니다. Chrome에서
  **`https://<내아이디>.github.io/iTamaPoke/iTamaPoke.apk`** 를 열면 파일이
...
알려줘서 D0으로 각자 만들게 하세요 — 10분이면 되고 비용도 없습니다.
