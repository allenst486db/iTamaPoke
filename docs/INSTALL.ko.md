...
|---|---|---|---|---|---|
| **D 웹앱** (안드로이드 APK는 미검증) | ★★★ 처음이라면 이것 | 폰, 무료 GitHub 계정 | fork 10분 + 설치 1분 | 없음 | ✗ |
...
> ⚠️ **안드로이드 APK는 실제 기기에서 검증되지 않았습니다.** CI에서 빌드는
> 되고 그 안의 웹 빌드는 아이폰에서 확인한 것과 같지만, APK를 실제로 폰에
> 설치해 플레이해본 적은 없습니다. 안 되면 아래 **앱 설치**(Chrome의 웹앱
> 방식)로 넘어가세요 — 그쪽은 아이폰과 같은 코드라 훨씬 안전합니다. 시도해
> 보셨다면 결과를 이슈로 남겨주시면 감사하겠습니다.
...
아이폰에서 홈 화면에 추가하는 과정은 이렇습니다.

| ① Safari로 링크 열기 | ② 공유 → 홈 화면에 추가 | ③ 홈 화면에 아이콘 생성 |
|---|---|---|
...
| 아래에 Safari 주소창이 보이는 상태 | ••• → **공유** → 아래로 스크롤 → **홈 화면에 추가** | 이제 이 아이콘으로 엽니다 |

> ⚠️ **스프라이트는 반드시 ③의 아이콘으로 연 앱에서 넣으세요.** 브라우저
> 탭(①)과 홈 화면 앱(③)은 **저장 공간이 서로 완전히 별개**입니다. 탭에서
> 파일을 넣고 아이콘으로 열면 캐릭터가 안 보이고, 그 반대도 마찬가지입니다.
> "분명히 넣었는데 안 나온다"는 경우는 거의 다 이것입니다.

설치 후에는 **항상 홈 화면 아이콘으로** 여세요. 세이브도 아이콘 쪽에만
쌓입니다.

<img src="img/web-app-standalone.png" alt="홈 화면 아이콘으로 연 앱: 아래에 Safari 주소창이 없음" width="230">

*아이콘으로 연 모습. 아래에 **주소창이 없으면** 제대로 연 것입니다. 위쪽
버튼 6개가 파일 불러오기·세이브·초기화입니다.*
...
- **Reset game…**: 세이브만 지우고 새 알부터. 스프라이트는 남습니다.
  잘못된 상태가 계속 보일 때 쓰세요.
- 첫 실행 뒤에는 **인터넷이 필요 없습니다.** 새 버전은 인터넷이 연결된
  상태로 다음에 열 때 자동으로 받아옵니다(화면이 한 번 새로고침될 수
  있음). 세이브와 스프라이트는 그대로 유지됩니다.
- 게임 아래에 **`build ...`** 줄이 있습니다. 지금 이 폰이 어느 버전을
  돌리고 있는지 알려주는 값으로, "고쳤다는데 그대로다" 싶을 때 이 값이
  바뀌었는지 보면 됩니다.
...
1. `mons` 폴더를 준비해 폰의 **파일 앱 → 나의 iPhone**(안드로이드: 내 파일 →
   ...
   구성하기](#mons-폴더-구성하기).
2. **홈 화면 아이콘으로** 앱을 열고(브라우저 탭이 아닙니다 — D2 참고) 위쪽
   **Load sprites…** 버튼을 탭합니다. 파일 선택 창이 뜨는데 처음에는
   **"최근 항목 없음"**이 나옵니다. 아래 **둘러보기**를 누르고
   **나의 iPhone** → **mons** 순으로 들어가세요.

   <img src="img/web-picker-browse.png" alt="파일 선택 창의 나의 iPhone 화면에 mons 폴더가 보임" width="230">

3. `mons` 폴더 안입니다. `.bin`만 선명하고 `.txt`·`.tpsave`는 흐리게
   보이는데 정상입니다 — 버튼마다 자기 확장자만 고를 수 있게 걸러줍니다.
   그래서 폴더에 다른 파일이 섞여 있어도 그냥 전부 선택하면 됩니다.

   <img src="img/web-picker-mons.png" alt="mons 폴더 안: p001.bin 등은 선택 가능, 도감 설명문과 세이브 파일은 흐리게 표시" width="230">

4. **파일 하나를 탭**하면 아래에 **전체 선택**이 나타납니다. 그걸 누르고
   오른쪽 위 **열기**를 탭하세요. (안드로이드: 파일 하나를 길게 누른 뒤
   **모두 선택** → **선택**.)

   <img src="img/web-picker-select-all.png" alt="파일 하나를 선택하자 아래에 전체 선택 버튼이 나타난 모습" width="230">
   <img src="img/web-picker-selected.png" alt="전체 선택을 눌러 .bin 파일이 모두 체크된 상태, 오른쪽 위 열기 버튼" width="230">

   *왼쪽: 파일 하나를 탭하면 **전체 선택**이 나타납니다. 오른쪽: 누르면
   `.bin`만 전부 체크되고(흐린 파일은 그대로), 오른쪽 위 **열기**를 누릅니다.*

5. 버튼 글씨가 "Loaded N file(s)"로 바뀌면 끝입니다. 캐릭터가 바로
   나타나고 앱 안에 저장되므로 한 번이면 됩니다.
6. 울음소리·도감 설명문도 같은 폴더에 있으면 **Load cries…**, **Load dex
   text…**에서 같은 방식으로 "전체 선택" 한 번씩.
...
| 고쳤다는 버그가 그대로 / 화면이 예전 그대로 | 게임 아래 **build** 값을 보세요. 안 바뀌었으면 옛 버전입니다 → **앱 전환기에서 위로 밀어 완전히 종료**한 뒤(홈으로 나가는 것만으로는 부족) 인터넷 켠 채 다시 열기. 그래도 안 되면 한 번 더 |
| 화면 아래 회색 줄에 `error: …` | 그 문장을 그대로 이슈로 남겨주세요. 진짜 버그입니다 |
| 아이폰에 "홈 화면에 추가"가 없음 | Safari가 아님 → 링크를 복사해 Safari에서 |
| 캐릭터가 "?" | **브라우저 탭에서 넣고 아이콘으로 연 경우가 가장 흔합니다** — 둘은 저장 공간이 별개입니다(D2). 아이콘으로 연 앱에서 D3을 다시 하세요 |
...
| 안드로이드 APK가 설치는 되는데 안 열리거나 흰 화면 | 이 APK는 실기기 검증이 안 됐습니다 → Chrome에서 웹 링크를 열고 ⋮ → **앱 설치**로 웹앱을 쓰세요. 증상을 이슈로 남겨주시면 고치겠습니다 |
