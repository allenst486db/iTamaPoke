...
1. 폰에서 [github.com/socquique/TamaPoke](https://github.com/socquique/TamaPoke)
   → 초록색 **Code** 버튼 → **Download ZIP**.
   - 아이폰: Safari가 **파일 → 다운로드**에 저장합니다. 파일 앱을 열고 zip을
     한 번 탭하면 `TamaPoke-main` 폴더로 풀립니다.
   - 안드로이드: **Downloads**에 받아지고, 파일 앱에서 탭하면 풀립니다.
2. 그 안에서 `tools` → `sdcard` → `mons` 폴더를 찾습니다. 종마다 `.bin`
   파일 하나씩 — `p001.bin`, `p002.bin`, …(일반)과 `ps001.bin`, …(이로치).
   어디로 옮길 필요 없습니다.
3. 아이콘으로 iTamaPoke를 열고 **"Load sprites…"**(오른쪽 위)를 탭합니다.
4. 파일 선택 창이 뜹니다. 그 `mons` 폴더로 들어가서 **`.bin` 파일을 전부 한
   번에 선택**합니다:
   - 아이폰: 오른쪽 위 **선택** → **모두 선택** → **열기**.
   - 안드로이드: 파일 하나를 길게 누른 뒤 **모두 선택** → **열기**(또는
     **선택**).
5. 화면 아래 개수 표시가 끝날 때까지 기다립니다. 캐릭터가 바로 나타나고
