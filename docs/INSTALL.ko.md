...
<img src="img/web-app-first-run.png" alt="처음 실행 화면: 위쪽에 파일 불러오기 버튼 6개, 아래에 스타터 선택 화면" width="230">

*처음 실행하면 스타터 선택 화면이 나옵니다. 위쪽 버튼 6개가 파일
불러오기·세이브·초기화입니다.*
...
1. `mons` 폴더를 준비해 폰의 **파일 앱 → 나의 iPhone**(안드로이드: 내 파일 →
   ...
   구성하기](#mons-폴더-구성하기).
2. 앱을 열고 위쪽 **Load sprites…** 버튼을 탭합니다. 파일 선택 창이 뜨는데
   처음에는 **"최근 항목 없음"**이 나옵니다. 아래 **둘러보기**를 누르고
