...
3. 목록 맨 위의 가장 최근 실행을 클릭합니다 — 초록색 체크 ✅가 있어야
   합니다.
4. 아래로 스크롤해서 **Artifacts** 아래 **TamaPoke-unsigned-ipa**를 클릭해
   다운로드합니다. `.zip`으로 받아지니 압축을 풀면 `TamaPoke-unsigned.ipa`
   파일이 나옵니다. 이게 앱 파일이고, 아직 내 폰용으로 서명은 안 된
   상태입니다(아래에서 설명).
...
2. `TamaPoke-unsigned.ipa`를 Sideloadly 창으로 드래그합니다.
...
이 아이폰에 애플워치가 페어링되어 있고 워치 앱도 설치했다면, 아이폰 앱을
잠깐 열어두면 같은 파일을 자동으로 워치에 보내려고 시도합니다. 그 다음
워치 앱을 열면 보통 바로 도착합니다 — 자세한 내용은 [메인
README](../README.md#스프라이트) 참고. 사이드로딩으로는 워치 앱 설치 자체가
이 전체 과정에서 가장 불안정한 부분이라는 점도 참고하세요(워치만큼은
아래 경로 C가 확실합니다).
