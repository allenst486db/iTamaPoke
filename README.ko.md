...
> 바꾼 리스킨 버전(다른 캐릭터, 같은 엔진)도 동일하게 안 됩니다.** 각자
> 본인 fork에서 본인 사본을 만들고, 누구도 남을 위해 호스팅하지 않습니다.
> [라이선스](#라이선스) 참고.

**그냥 플레이하고 싶다면:** 이 저장소를 Fork하고, GitHub Pages를 켜고,
**Build (browser)** 워크플로를 한 번 실행하세요 — 약 10분, 설치할 프로그램
없음. 그러면 본인 링크(`https://<내아이디>.github.io/iTamaPoke/`)에 웹앱
(아이폰: Safari → 공유 → *홈 화면에 추가*)과 안드로이드 `iTamaPoke.apk`가
생깁니다. Mac도, 개발자 계정도, 만료도 없고 그 뒤로는 폰에서 오프라인으로
돌아갑니다. 캐릭터 그림은 직접 넣어야 합니다 — 빌드에는 일부러 하나도 안
들어 있습니다. 단계별: [설치 가이드 경로
D](docs/INSTALL.ko.md#경로-d--내-fork에서-만드는-웹앱--안드로이드-앱-mac-없이-개발자-계정-없이).

> ⚠️ **안드로이드 APK는 실제 기기에서 테스트해본 적이 없습니다.** CI에서
> 빌드는 되고 그 안의 웹 빌드는 아이폰에서 검증한 것과 같지만, APK를 실제로
> 설치해 플레이해본 사람은 아직 없습니다 — 미검증으로 봐주세요.
...
그대로 씁니다. 별명도 한글로 입력할 수 있습니다 — 이름 변경 키보드에 ABC/한
전환이 있고 자모를 음절로 조합합니다(`Sources/Core/hangul.cpp`, 두 빌드가
공유). **아직 실제 아이폰·워치에서 검증하지 않았습니다** — 문자열
...
| | 필요한 것 | 지원 OS | 애플워치 | 비고 |
|---|---|---|---|---|
| **내 fork에서 만든 웹앱** (`browser_ver/`) | 무료 GitHub 계정, 폰 | 아이폰 / 아이패드 / 안드로이드 / 데스크톱 브라우저 | 없음 | 같은 게임, 같은 C++ 코어를 WebAssembly로 컴파일; GitHub가 내 계정 아래에 빌드, Safari/Chrome에서 설치, 오프라인 동작, 만료 없음. 세이브와 스프라이트는 기기 안에 |
| **안드로이드 `.apk`** (같은 사이트) ⚠️ *실기기 미검증* | 위와 동일 | Android 8+ | 없음 | 웹 빌드를 진짜 앱(WebView 셸)으로 감싼 것, CI가 디버그 서명; 덮어 설치로 업데이트. CI 빌드는 되지만 실제 폰에서 돌려본 적은 없음 |
...
- **이 포팅 자체의 코드**(Swift/watchOS 레이어, 브라우저 빌드, 스크립트,
  ...
  커스텀 라이선스입니다. 개인 이용과 수정은 자유롭고, 본인 fork의
  웹앱/안드로이드 빌드를 본인 계정에 본인용으로 올리는 것도 됩니다. 어떤
  빌드든(리스킨 포함) 남에게 넘기거나 남을 어떤 빌드로 안내하려면 먼저
  저작권자의 허락이 필요합니다. 길지 않으니 꼭 읽어보세요.
- **fork의 사이트가 실제로 내보내는 것**은 이 저장소에 있는 것과 같습니다:
